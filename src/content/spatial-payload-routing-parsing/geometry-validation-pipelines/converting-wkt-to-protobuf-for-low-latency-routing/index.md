...
description: "Replace verbose WKT geometry strings with compact Protobuf binary payloads: reduce payload size 60–85% and drop deserialization to sub-millisecond ranges."
---
...
def _flatten_coords(geom: BaseGeometry) -> List[float]:
    """Extract all (x, y) pairs from a geometry into a flat list."""
...
- **Schema Evolution:** Avoid renaming fields or changing types in production. Use `reserved` tags for deprecated fields and document SRID assumptions explicitly. Protobuf's backward compatibility relies on stable field numbers — never reuse them.
...
- **Benchmark Expectations:** On standard x86 cloud instances, expect approximately 0.4–0.8 ms serialization for a 500-vertex LINESTRING, with payload sizes dropping from ~12 KB (WKT) to ~4 KB (Protobuf). Deserialization typically runs at 0.1–0.3 ms when using memoryview-backed parsers.
