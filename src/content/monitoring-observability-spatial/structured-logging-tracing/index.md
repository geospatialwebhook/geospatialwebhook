...
<figure class="fig">
<svg viewBox="0 0 760 232" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Context bleeding between concurrent requests when stored globally, versus isolated per-task contextvars">
<title>Why the fingerprint lives in a contextvar, not a module global</title>
<desc>Two webhook requests are handled concurrently on one event loop. With the spatial context held in a module-level global, request A binds its Berlin fingerprint, awaits an I/O call, and request B binds its Tokyo fingerprint over the top; when A resumes and logs, it emits Tokyo's bounding box and H3 cell attached to Berlin's error. The line is well-formed, the field names are right and the values belong to a different request entirely, so an engineer triaging a Berlin failure is sent to look at Tokyo. With a contextvar each task gets its own copy at the first await, so A resumes with its own fingerprint intact. The one place this still needs care is asyncio.create_task, which copies the context at creation: a task spawned before the context is bound captures the empty version, so bind at ingress before anything is scheduled.</desc>
<rect x="0" y="0" width="760" height="232" fill="var(--fig-bg)"/>
<text x="14" y="20" font-size="10.5" font-weight="600" fill="var(--fig-rose-edge)">Module-level global — one slot, two requests</text>
<rect x="90" y="30" width="110" height="20" rx="3" fill="var(--fig-mint)" stroke="var(--fig-mint-edge)" stroke-width="1.1"/>
<text x="100" y="44" font-size="8.5" fill="var(--fig-ink)">A binds Berlin</text>
<rect x="204" y="30" width="90" height="20" rx="3" fill="var(--fig-earth)" stroke="var(--fig-earth-edge)" stroke-width="1"/>
<text x="214" y="44" font-size="8.5" fill="var(--fig-ink-soft)">A awaits I/O</text>
<rect x="298" y="30" width="118" height="20" rx="3" fill="var(--fig-peach)" stroke="var(--fig-peach-edge)" stroke-width="1.1"/>
<text x="308" y="44" font-size="8" fill="var(--fig-ink)">B binds Tokyo — overwrites</text>
<rect x="420" y="30" width="200" height="20" rx="3" fill="var(--fig-rose)" stroke="var(--fig-rose-edge)" stroke-width="1.4"/>
<text x="430" y="44" font-size="8.5" fill="var(--fig-ink)">A resumes and logs — with Tokyo's fingerprint</text>
<text x="14" y="72" font-size="8.5" font-family="monospace" fill="var(--fig-rose-edge)">{"event":"validation failed", "feature_id":4471, "h3_r7":"872f5a…Tokyo", "bbox":[139.6,35.6,139.8,35.8]}</text>
<text x="14" y="88" font-size="9" fill="var(--fig-ink-soft)">Well-formed, correctly named, and pointing at the wrong hemisphere. Triage for a Berlin failure begins in Tokyo.</text>
<line x1="14" y1="102" x2="746" y2="102" stroke="var(--fig-line-soft)" stroke-width="1"/>
<text x="14" y="124" font-size="10.5" font-weight="600" fill="var(--fig-mint-edge)">contextvars — a copy per task</text>
<rect x="90" y="134" width="110" height="20" rx="3" fill="var(--fig-mint)" stroke="var(--fig-mint-edge)" stroke-width="1.1"/>
<text x="100" y="148" font-size="8.5" fill="var(--fig-ink)">A binds Berlin</text>
<rect x="204" y="134" width="90" height="20" rx="3" fill="var(--fig-earth)" stroke="var(--fig-earth-edge)" stroke-width="1"/>
<text x="214" y="148" font-size="8.5" fill="var(--fig-ink-soft)">A awaits I/O</text>
<rect x="298" y="134" width="118" height="20" rx="3" fill="var(--fig-peach)" stroke="var(--fig-peach-edge)" stroke-width="1.1"/>
<text x="308" y="148" font-size="8" fill="var(--fig-ink)">B binds Tokyo — own copy</text>
<rect x="420" y="134" width="200" height="20" rx="3" fill="var(--fig-mint)" stroke="var(--fig-mint-edge)" stroke-width="1.4"/>
<text x="430" y="148" font-size="8.5" fill="var(--fig-ink)">A resumes — Berlin fingerprint intact</text>
<rect x="14" y="168" width="732" height="58" rx="6" fill="var(--fig-gold)" stroke="var(--fig-gold-edge)" stroke-width="1.3"/>
<text x="26" y="186" font-size="10" font-weight="600" fill="var(--fig-ink)">The one case contextvars still gets wrong</text>
<text x="26" y="204" font-size="9" fill="var(--fig-ink-soft)">asyncio.create_task copies the context at creation time, so a task spawned before the fingerprint is bound captures the empty version and</text>
<text x="26" y="218" font-size="9" fill="var(--fig-ink-soft)">logs without spatial fields forever after. Bind at ingress, before anything is scheduled — the same ordering the trace context needs.</text>
</svg>
<figcaption><b>Figure 1.</b> A cross-contaminated log line is worse than a missing one: it is structurally valid, so it survives every schema check and sends triage to the wrong region.</figcaption>
</figure>

<figure class="fig">
<svg viewBox="0 0 760 240" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="A log line without spatial context compared with one carrying a bounded spatial fingerprint">
<title>What a spatial fingerprint adds to a log line</title>
<desc>The same validation failure logged two ways. Without context the line reads only that geometry validation failed with a self-intersection at level error, which tells an on-call engineer nothing about which feature, which region, which upstream system or which delivery, so triage begins by grepping other systems for the timestamp. With the fingerprint bound once at ingress and merged by a structlog processor, the same line carries the correlation id, the feature id, the H3 cell at resolution seven, the bounding box, the source EPSG code and the idempotency key — about two hundred and forty bytes. That is enough to group failures by region, join to the trace, find the sibling deliveries of the same event and identify the upstream system, all without the call site repeating a single field and without ever serialising the geometry itself into the log.</desc>
<rect x="0" y="0" width="760" height="240" fill="var(--fig-bg)"/>
<rect x="14" y="30" width="352" height="88" rx="7" fill="var(--fig-rose)" stroke="var(--fig-rose-edge)" stroke-width="1.5"/>
<text x="26" y="50" font-size="10" font-weight="600" fill="var(--fig-ink)">Without context</text>
<text x="26" y="70" font-size="8.5" font-family="monospace" fill="var(--fig-ink-soft)">{"level":"error",</text>
<text x="26" y="84" font-size="8.5" font-family="monospace" fill="var(--fig-ink-soft)"> "event":"geometry validation failed",</text>
<text x="26" y="98" font-size="8.5" font-family="monospace" fill="var(--fig-ink-soft)"> "error":"self-intersection"}</text>
<text x="26" y="113" font-size="8.5" fill="var(--fig-rose-edge)">Triage starts by grepping other systems for the timestamp.</text>
<rect x="386" y="30" width="360" height="88" rx="7" fill="var(--fig-mint)" stroke="var(--fig-mint-edge)" stroke-width="1.5"/>
<text x="398" y="50" font-size="10" font-weight="600" fill="var(--fig-ink)">With the fingerprint — ~240 bytes</text>
<text x="398" y="68" font-size="8.5" font-family="monospace" fill="var(--fig-ink-soft)">"correlation_id":"01JQ…", "feature_id":4471,</text>
<text x="398" y="82" font-size="8.5" font-family="monospace" fill="var(--fig-ink-soft)">"h3_r7":"871f1d4ffffffff", "source_epsg":27700,</text>
<text x="398" y="96" font-size="8.5" font-family="monospace" fill="var(--fig-ink-soft)">"bbox":[13.3,52.4,13.5,52.6], "idem_key":"9f2c…"</text>
<text x="398" y="113" font-size="8.5" fill="var(--fig-mint-edge)">Bound once at ingress; every line inherits it.</text>
<text x="14" y="142" font-size="10" font-weight="600" fill="var(--fig-ink)">What each field unlocks during an incident</text>
<rect x="14" y="152" width="176" height="46" rx="5" fill="var(--fig-earth)" stroke="var(--fig-earth-edge)" stroke-width="1.2"/>
<text x="24" y="169" font-size="9" font-weight="600" fill="var(--fig-ink)">h3_r7 + bbox</text>
<text x="24" y="185" font-size="8.5" fill="var(--fig-ink-soft)">group failures by region —</text>
<text x="24" y="195" font-size="8.5" fill="var(--fig-ink-soft)">is one city broken, or all?</text>
<rect x="198" y="152" width="176" height="46" rx="5" fill="var(--fig-earth)" stroke="var(--fig-earth-edge)" stroke-width="1.2"/>
<text x="208" y="169" font-size="9" font-weight="600" fill="var(--fig-ink)">correlation_id</text>
<text x="208" y="185" font-size="8.5" fill="var(--fig-ink-soft)">join straight to the trace</text>
<text x="208" y="195" font-size="8.5" fill="var(--fig-ink-soft)">waterfall for this event</text>
<rect x="382" y="152" width="176" height="46" rx="5" fill="var(--fig-earth)" stroke="var(--fig-earth-edge)" stroke-width="1.2"/>
<text x="392" y="169" font-size="9" font-weight="600" fill="var(--fig-ink)">source_epsg</text>
<text x="392" y="185" font-size="8.5" fill="var(--fig-ink-soft)">identify which upstream</text>
<text x="392" y="195" font-size="8.5" fill="var(--fig-ink-soft)">system started sending this</text>
<rect x="566" y="152" width="180" height="46" rx="5" fill="var(--fig-earth)" stroke="var(--fig-earth-edge)" stroke-width="1.2"/>
<text x="576" y="169" font-size="9" font-weight="600" fill="var(--fig-ink)">idem_key</text>
<text x="576" y="185" font-size="8.5" fill="var(--fig-ink-soft)">find every redelivery of</text>
<text x="576" y="195" font-size="8.5" fill="var(--fig-ink-soft)">this same event</text>
<rect x="14" y="208" width="732" height="26" rx="5" fill="var(--fig-gold)" stroke="var(--fig-gold-edge)" stroke-width="1.2"/>
<text x="26" y="225" font-size="9.5" fill="var(--fig-ink)">Note what is absent: the geometry. The fingerprint is bounded in size, so log volume stays flat whether a payload has 4 vertices or 400,000.</text>
</svg>
<figcaption><b>Figure 2.</b> The fingerprint is chosen so its size does not scale with the payload. That is what makes it safe to attach to <em>every</em> line rather than only to errors — where you would need it before knowing you did.</figcaption>
</figure>
...
<figcaption><b>Figure 3.</b> Trace waterfall for one spatial webhook event</figcaption>
</figure>
