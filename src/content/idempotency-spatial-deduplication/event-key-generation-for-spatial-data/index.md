...
<figure class="fig">
<svg viewBox="0 0 760 250" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Ground distance represented by each decimal place of latitude and longitude, and the band where a rounding policy belongs">
<title>What each decimal place of a coordinate is worth</title>
<desc>Ground distance at the equator for one unit in each decimal place of a WGS84 coordinate. Four decimal places is about eleven metres, five is about 1.1 metres, six is about eleven centimetres, seven is about 1.1 centimetres and eight is about 1.1 millimetres. Consumer GPS hardware is accurate to roughly three to fifteen metres and cell or wi-fi triangulation to fifty to two hundred metres, so any digit beyond the fifth is describing noise rather than position. Rounding at four decimal places sits inside GPS error and is the usual choice for tracker telemetry; six is right for surveyed parcel boundaries; eight preserves float noise and defeats the idempotency gate entirely, because two deliveries of a stationary sensor differ in the last digit and hash differently. The rounding place is therefore the distance below which the system declares two readings to be the same location, and it has to be chosen against the sensor rather than against the format.</desc>
<rect x="0" y="0" width="760" height="250" fill="var(--fig-bg)"/>
<rect x="196" y="30" width="200" height="150" fill="var(--fig-mint)" opacity="0.45"/>
<text x="296" y="24" text-anchor="middle" font-size="9.5" font-weight="600" fill="var(--fig-mint-edge)">where a rounding policy belongs</text>
<line x1="60" y1="180" x2="700" y2="180" stroke="var(--fig-line)" stroke-width="1.3"/>
<line x1="100" y1="174" x2="100" y2="186" stroke="var(--fig-line)" stroke-width="1.1"/>
<line x1="196" y1="174" x2="196" y2="186" stroke="var(--fig-line)" stroke-width="1.1"/>
<line x1="296" y1="174" x2="296" y2="186" stroke="var(--fig-line)" stroke-width="1.1"/>
<line x1="396" y1="174" x2="396" y2="186" stroke="var(--fig-line)" stroke-width="1.1"/>
<line x1="496" y1="174" x2="496" y2="186" stroke="var(--fig-line)" stroke-width="1.1"/>
<line x1="596" y1="174" x2="596" y2="186" stroke="var(--fig-line)" stroke-width="1.1"/>
<text x="100" y="198" text-anchor="middle" font-size="9" fill="var(--fig-ink-soft)">3 d.p.</text>
<text x="196" y="198" text-anchor="middle" font-size="9" font-weight="700" fill="var(--fig-ink)">4 d.p.</text>
<text x="296" y="198" text-anchor="middle" font-size="9" font-weight="700" fill="var(--fig-ink)">5 d.p.</text>
<text x="396" y="198" text-anchor="middle" font-size="9" font-weight="700" fill="var(--fig-ink)">6 d.p.</text>
<text x="496" y="198" text-anchor="middle" font-size="9" fill="var(--fig-ink-soft)">7 d.p.</text>
<text x="596" y="198" text-anchor="middle" font-size="9" fill="var(--fig-ink-soft)">8 d.p.</text>
<text x="100" y="214" text-anchor="middle" font-size="8.5" fill="var(--fig-ink-soft)">110 m</text>
<text x="196" y="214" text-anchor="middle" font-size="8.5" fill="var(--fig-ink-soft)">11 m</text>
<text x="296" y="214" text-anchor="middle" font-size="8.5" fill="var(--fig-ink-soft)">1.1 m</text>
<text x="396" y="214" text-anchor="middle" font-size="8.5" fill="var(--fig-ink-soft)">11 cm</text>
<text x="496" y="214" text-anchor="middle" font-size="8.5" fill="var(--fig-ink-soft)">1.1 cm</text>
<text x="596" y="214" text-anchor="middle" font-size="8.5" fill="var(--fig-ink-soft)">1.1 mm</text>
<rect x="60" y="44" width="236" height="24" rx="4" fill="var(--fig-earth)" stroke="var(--fig-earth-edge)" stroke-width="1.2"/>
<text x="70" y="60" font-size="9" fill="var(--fig-ink)">cell / wi-fi triangulation — 50–200 m error</text>
<rect x="160" y="74" width="176" height="24" rx="4" fill="var(--fig-earth)" stroke="var(--fig-earth-edge)" stroke-width="1.2"/>
<text x="170" y="90" font-size="9" fill="var(--fig-ink)">consumer GPS — 3–15 m error</text>
<rect x="336" y="104" width="160" height="24" rx="4" fill="var(--fig-mint)" stroke="var(--fig-mint-edge)" stroke-width="1.2"/>
<text x="346" y="120" font-size="9" fill="var(--fig-ink)">surveyed parcel — round at 6</text>
<rect x="496" y="134" width="212" height="24" rx="4" fill="var(--fig-rose)" stroke="var(--fig-rose-edge)" stroke-width="1.4"/>
<text x="506" y="150" font-size="9" fill="var(--fig-ink)">float noise lives here — never hash raw</text>
<rect x="14" y="228" width="732" height="0" fill="none"/>
<text x="14" y="238" font-size="9" fill="var(--fig-ink-soft)">Beyond 5 d.p. a consumer-GPS coordinate is describing its own error, not the device's position — so hashing those digits hashes noise.</text>
</svg>
<figcaption><b>Figure 2.</b> Precision policy is where deduplication is actually decided. Pick the rounding place from the sensor's real accuracy: keep digits your hardware cannot resolve and every redelivery of a stationary device hashes differently.</figcaption>
</figure>
...
<figure class="fig">
<svg viewBox="0 0 760 236" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Reprojecting before rounding versus rounding before reprojecting, and why the order changes the key">
<title>Reproject first, then round — the order is not interchangeable</title>
<desc>The same Web Mercator point is put through the two pipeline orderings. Reprojecting first converts 13,627,361.03 and 4,547,679.25 metres to 122.4194 and 37.7749 degrees, then rounds at six decimal places, giving a stable key; a second delivery whose metre values differ by a few centimetres reprojects to degree values that round to exactly the same digits, so both hash identically. Rounding first quantises in metres — a unit a thousand times coarser than the degree rounding downstream expects — and the reprojection then spreads that quantisation unevenly across latitude, so two deliveries of the same physical point can land on different rounded degrees and produce different keys. The rule is that every normalisation step must run in the units the next step assumes, and rounding is only meaningful once the coordinate is in its final CRS.</desc>
<rect x="0" y="0" width="760" height="236" fill="var(--fig-bg)"/>
<defs><marker id="ko-a" markerWidth="7" markerHeight="6" refX="6" refY="3" orient="auto"><path d="M0,0 L7,3 L0,6 Z" fill="var(--fig-line)"/></marker></defs>
<text x="14" y="20" font-size="10.5" font-weight="600" fill="var(--fig-mint-edge)">Reproject → round → hash</text>
<rect x="14" y="30" width="168" height="40" rx="5" fill="var(--fig-earth)" stroke="var(--fig-earth-edge)" stroke-width="1.2"/>
<text x="98" y="47" text-anchor="middle" font-size="8.5" fill="var(--fig-ink)">EPSG:3857 metres</text>
<text x="98" y="60" text-anchor="middle" font-size="8.5" fill="var(--fig-ink-soft)">13627361.03, 4547679.25</text>
<line x1="184" y1="50" x2="212" y2="50" stroke="var(--fig-line)" stroke-width="1.3" marker-end="url(#ko-a)"/>
<rect x="216" y="30" width="168" height="40" rx="5" fill="var(--fig-mint)" stroke="var(--fig-mint-edge)" stroke-width="1.2"/>
<text x="300" y="47" text-anchor="middle" font-size="8.5" fill="var(--fig-ink)">EPSG:4326 degrees</text>
<text x="300" y="60" text-anchor="middle" font-size="8.5" fill="var(--fig-ink-soft)">122.41940017, 37.77490042</text>
<line x1="386" y1="50" x2="414" y2="50" stroke="var(--fig-line)" stroke-width="1.3" marker-end="url(#ko-a)"/>
<rect x="418" y="30" width="150" height="40" rx="5" fill="var(--fig-mint)" stroke="var(--fig-mint-edge)" stroke-width="1.4"/>
<text x="493" y="47" text-anchor="middle" font-size="8.5" fill="var(--fig-ink)">round 6 d.p.</text>
<text x="493" y="60" text-anchor="middle" font-size="8.5" fill="var(--fig-ink-soft)">122.419400, 37.774900</text>
<line x1="570" y1="50" x2="598" y2="50" stroke="var(--fig-line)" stroke-width="1.3" marker-end="url(#ko-a)"/>
<rect x="602" y="30" width="144" height="40" rx="5" fill="var(--fig-mint)" stroke="var(--fig-mint-edge)" stroke-width="1.5"/>
<text x="674" y="54" text-anchor="middle" font-size="9" font-weight="600" fill="var(--fig-ink)">stable key ✓</text>
<text x="14" y="94" font-size="9" fill="var(--fig-ink-soft)">A redelivery a few centimetres off in metres reprojects to degrees that round to the same six digits — so it hashes identically.</text>
<line x1="14" y1="110" x2="746" y2="110" stroke="var(--fig-line-soft)" stroke-width="1"/>
<text x="14" y="132" font-size="10.5" font-weight="600" fill="var(--fig-rose-edge)">Round → reproject → hash</text>
<rect x="14" y="142" width="168" height="40" rx="5" fill="var(--fig-earth)" stroke="var(--fig-earth-edge)" stroke-width="1.2"/>
<text x="98" y="159" text-anchor="middle" font-size="8.5" fill="var(--fig-ink)">EPSG:3857 metres</text>
<text x="98" y="172" text-anchor="middle" font-size="8.5" fill="var(--fig-ink-soft)">13627361.03, 4547679.25</text>
<line x1="184" y1="162" x2="212" y2="162" stroke="var(--fig-line)" stroke-width="1.3" marker-end="url(#ko-a)"/>
<rect x="216" y="142" width="168" height="40" rx="5" fill="var(--fig-rose)" stroke="var(--fig-rose-edge)" stroke-width="1.3"/>
<text x="300" y="159" text-anchor="middle" font-size="8.5" fill="var(--fig-ink)">round 6 d.p. — of a metre</text>
<text x="300" y="172" text-anchor="middle" font-size="8.5" fill="var(--fig-ink-soft)">quantises 1000× too fine to help</text>
<line x1="386" y1="162" x2="414" y2="162" stroke="var(--fig-line)" stroke-width="1.3" marker-end="url(#ko-a)"/>
<rect x="418" y="142" width="150" height="40" rx="5" fill="var(--fig-rose)" stroke="var(--fig-rose-edge)" stroke-width="1.3"/>
<text x="493" y="159" text-anchor="middle" font-size="8.5" fill="var(--fig-ink)">reproject → degrees</text>
<text x="493" y="172" text-anchor="middle" font-size="8.5" fill="var(--fig-ink-soft)">error spreads with latitude</text>
<line x1="570" y1="162" x2="598" y2="162" stroke="var(--fig-line)" stroke-width="1.3" marker-end="url(#ko-a)"/>
<rect x="602" y="142" width="144" height="40" rx="5" fill="var(--fig-rose)" stroke="var(--fig-rose-edge)" stroke-width="1.5"/>
<text x="674" y="166" text-anchor="middle" font-size="9" font-weight="600" fill="var(--fig-ink)">key drifts ✗</text>
<rect x="14" y="200" width="732" height="30" rx="5" fill="var(--fig-gold)" stroke="var(--fig-gold-edge)" stroke-width="1.2"/>
<text x="26" y="219" font-size="9.5" fill="var(--fig-ink)">Rounding is only meaningful in the CRS the key is defined in. Every normalisation step has to run in the units the next step assumes.</text>
</svg>
<figcaption><b>Figure 3.</b> Both pipelines contain the same two operations. Only one produces a stable key, because a decimal place means eleven centimetres in degrees and a micrometre in Web Mercator metres.</figcaption>
</figure>
