  ...
  "@graph": [
    ...
    },
    {
      ...
      "itemListElement": [
        ...
        {"@type": "ListItem", "position": 2, "name": "Cache-Backed Idempotency Checks", "item": "https://geospatialwebhook.com/idempotency-spatial-deduplication/cache-backed-idempotency-checks/"},
        {"@type": "ListItem", "position": 3, "name": "Using Redis to Cache Spatial Webhook Signatures", "item": "https://geospatialwebhook.com/idempotency-spatial-deduplication/cache-backed-idempotency-checks/using-redis-to-cache-spatial-webhook-signatures/"}
      ]
    },
    {
      "@type": "HowTo",
...
3. **Mixed CRS payloads colliding after normalization.** A feature in EPSG:3857 (Web Mercator) and the same feature re-projected to EPSG:4326 (WGS84) will have very different coordinate values, so they will not collide in Redis — but they *should* be treated as duplicates if they represent the same real-world event. Always normalize to a single CRS before hashing; see [Handling Mixed CRS Payloads in Python Event Handlers](/spatial-payload-routing-parsing/crs-normalization-strategies/handling-mixed-crs-payloads-in-python-event-handlers/) for a projection pipeline.
...
- [Cache-Backed Idempotency Checks for Geospatial Webhooks](/idempotency-spatial-deduplication/cache-backed-idempotency-checks/) — parent: the full pipeline from ingestion to conditional execution
- [Generating Deterministic Idempotency Keys for GeoJSON Events](/idempotency-spatial-deduplication/event-key-generation-for-spatial-data/generating-deterministic-idempotency-keys-for-geojson-events/) — deep dive on BLAKE2b vs SHA-256 and recursive float normalization
- [Idempotency & Spatial Deduplication](/idempotency-spatial-deduplication/) — architectural overview of exactly-once semantics for geospatial event streams
