...
<figure class="fig">
<svg viewBox="0 0 760 224" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="A retry window outliving the idempotency TTL, and the margin that keeps them ordered">
<title>The retry window must end before the idempotency key does</title>
<desc>A provider's retry schedule runs attempts at one minute, thirty minutes, four hours, twenty-four hours and finally seventy-two hours. With the idempotency key given a forty-eight hour time-to-live, the first four attempts find the key and short-circuit correctly, but the seventy-two hour attempt arrives twenty-four hours after the key expired: it reads no key, concludes the delivery is new, and writes the spatial feature a second time. Nothing errors and the duplicate appears three days after the original, long after anyone connects it to the incident. Deriving the time-to-live from the window instead — the full seventy-two hour window plus a twelve hour margin, giving eighty-four hours — keeps every attempt inside the key's lifetime with room for clock skew and a delayed queue. The margin matters because the last attempt is the one most likely to arrive late, and it is also the one whose duplicate is hardest to trace.</desc>
<rect x="0" y="0" width="760" height="224" fill="var(--fig-bg)"/>
<rect x="70" y="40" width="404" height="24" fill="var(--fig-gold)" opacity="0.55"/>
<text x="272" y="34" text-anchor="middle" font-size="9.5" font-weight="600" fill="var(--fig-gold-edge)">idempotency TTL 48 h — expires here</text>
<rect x="70" y="76" width="660" height="24" fill="var(--fig-mint)" opacity="0.5"/>
<text x="400" y="93" text-anchor="middle" font-size="9.5" font-weight="600" fill="var(--fig-mint-edge)">TTL = window + 12 h margin = 84 h</text>
<line x1="70" y1="126" x2="700" y2="126" stroke="var(--fig-line)" stroke-width="1.3"/>
<circle cx="76" cy="126" r="4" fill="var(--fig-mint-edge)"/>
<circle cx="106" cy="126" r="4" fill="var(--fig-mint-edge)"/>
<circle cx="160" cy="126" r="4" fill="var(--fig-mint-edge)"/>
<circle cx="266" cy="126" r="4" fill="var(--fig-mint-edge)"/>
<circle cx="632" cy="126" r="5.5" fill="var(--fig-rose-edge)"/>
<text x="76" y="144" text-anchor="middle" font-size="8.5" fill="var(--fig-ink-soft)">1 min</text>
<text x="106" y="144" text-anchor="middle" font-size="8.5" fill="var(--fig-ink-soft)">30 min</text>
<text x="160" y="144" text-anchor="middle" font-size="8.5" fill="var(--fig-ink-soft)">4 h</text>
<text x="266" y="144" text-anchor="middle" font-size="8.5" fill="var(--fig-ink-soft)">24 h</text>
<text x="632" y="144" text-anchor="middle" font-size="8.5" font-weight="700" fill="var(--fig-rose-edge)">72 h</text>
<line x1="474" y1="40" x2="474" y2="132" stroke="var(--fig-gold-edge)" stroke-width="1.6" stroke-dasharray="5,3"/>
<text x="632" y="160" text-anchor="middle" font-size="8.5" fill="var(--fig-rose-edge)">key gone 24 h ago ⇒ duplicate write</text>
<rect x="14" y="176" width="366" height="42" rx="5" fill="var(--fig-rose)" stroke="var(--fig-rose-edge)" stroke-width="1.3"/>
<text x="26" y="193" font-size="9.5" font-weight="600" fill="var(--fig-ink)">Two independent constants drift apart</text>
<text x="26" y="210" font-size="9" fill="var(--fig-ink-soft)">Someone extends the window; nobody remembers the TTL.</text>
<rect x="394" y="176" width="352" height="42" rx="5" fill="var(--fig-mint)" stroke="var(--fig-mint-edge)" stroke-width="1.3"/>
<text x="406" y="193" font-size="9.5" font-weight="600" fill="var(--fig-ink)">ttl = total_window + margin</text>
<text x="406" y="210" font-size="9" fill="var(--fig-ink-soft)">Derived, so a config change cannot separate them.</text>
</svg>
<figcaption><b>Figure 2.</b> Derive the TTL from the window rather than setting both by hand. The duplicate this prevents arrives days later, which is precisely when nobody will connect it back to a retry.</figcaption>
</figure>
...
<figure class="fig">
<svg viewBox="0 0 760 230" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Retry attempts multiplying through a three-hop chain when each hop keeps its own full budget">
<title>Retry budgets multiply through a chain; they do not add</title>
<desc>One edge delivery fans out through a CRS normalisation service and then a tile builder. If each hop independently keeps a budget of five attempts, the edge makes five calls, each of which the normaliser retries five times for twenty-five, and each of those the tile builder retries five times for one hundred and twenty-five downstream calls from a single event. The multiplication is invisible from any one service's dashboard, because each hop correctly reports that it made five attempts. Budgeting end to end instead — three attempts at the edge, two at the normaliser, two at the builder — caps the same chain at twelve calls. The rule that makes this work is that every inner hop must get a strictly smaller window than its caller: if an inner retry can still be running when the outer one gives up and retries, the outer attempt starts a second inner chain on top of the first.</desc>
<rect x="0" y="0" width="760" height="230" fill="var(--fig-bg)"/>
<defs><marker id="ba-a" markerWidth="7" markerHeight="6" refX="6" refY="3" orient="auto"><path d="M0,0 L7,3 L0,6 Z" fill="var(--fig-line)"/></marker></defs>
<text x="14" y="20" font-size="10.5" font-weight="600" fill="var(--fig-rose-edge)">Each hop keeps its own budget of 5</text>
<rect x="14" y="30" width="130" height="44" rx="6" fill="var(--fig-rose)" stroke="var(--fig-rose-edge)" stroke-width="1.3"/>
<text x="79" y="49" text-anchor="middle" font-size="9.5" font-weight="600" fill="var(--fig-ink)">edge</text>
<text x="79" y="64" text-anchor="middle" font-size="9" fill="var(--fig-ink-soft)">5 attempts</text>
<line x1="146" y1="52" x2="180" y2="52" stroke="var(--fig-line)" stroke-width="1.3" marker-end="url(#ba-a)"/>
<rect x="184" y="30" width="150" height="44" rx="6" fill="var(--fig-rose)" stroke="var(--fig-rose-edge)" stroke-width="1.3"/>
<text x="259" y="49" text-anchor="middle" font-size="9.5" font-weight="600" fill="var(--fig-ink)">CRS normalisation</text>
<text x="259" y="64" text-anchor="middle" font-size="9" fill="var(--fig-ink-soft)">5 × 5 = 25 calls</text>
<line x1="336" y1="52" x2="370" y2="52" stroke="var(--fig-line)" stroke-width="1.3" marker-end="url(#ba-a)"/>
<rect x="374" y="30" width="150" height="44" rx="6" fill="var(--fig-rose)" stroke="var(--fig-rose-edge)" stroke-width="1.6"/>
<text x="449" y="49" text-anchor="middle" font-size="9.5" font-weight="600" fill="var(--fig-ink)">tile builder</text>
<text x="449" y="64" text-anchor="middle" font-size="9" fill="var(--fig-ink-soft)">25 × 5 = 125 calls</text>
<rect x="540" y="30" width="206" height="44" rx="6" fill="var(--fig-rose)" stroke="var(--fig-rose-edge)" stroke-width="1.4"/>
<text x="552" y="48" font-size="9" font-weight="600" fill="var(--fig-ink)">125 calls from one event</text>
<text x="552" y="64" font-size="8.5" fill="var(--fig-ink-soft)">and every dashboard reads "5 attempts"</text>
<line x1="14" y1="92" x2="746" y2="92" stroke="var(--fig-line-soft)" stroke-width="1"/>
<text x="14" y="112" font-size="10.5" font-weight="600" fill="var(--fig-mint-edge)">One budget divided across the chain</text>
<rect x="14" y="122" width="130" height="44" rx="6" fill="var(--fig-mint)" stroke="var(--fig-mint-edge)" stroke-width="1.3"/>
<text x="79" y="141" text-anchor="middle" font-size="9.5" font-weight="600" fill="var(--fig-ink)">edge</text>
<text x="79" y="156" text-anchor="middle" font-size="9" fill="var(--fig-ink-soft)">3 attempts · 60 min</text>
<line x1="146" y1="144" x2="180" y2="144" stroke="var(--fig-line)" stroke-width="1.3" marker-end="url(#ba-a)"/>
<rect x="184" y="122" width="150" height="44" rx="6" fill="var(--fig-mint)" stroke="var(--fig-mint-edge)" stroke-width="1.3"/>
<text x="259" y="141" text-anchor="middle" font-size="9.5" font-weight="600" fill="var(--fig-ink)">CRS normalisation</text>
<text x="259" y="156" text-anchor="middle" font-size="9" fill="var(--fig-ink-soft)">2 attempts · 15 min</text>
<line x1="336" y1="144" x2="370" y2="144" stroke="var(--fig-line)" stroke-width="1.3" marker-end="url(#ba-a)"/>
<rect x="374" y="122" width="150" height="44" rx="6" fill="var(--fig-mint)" stroke="var(--fig-mint-edge)" stroke-width="1.3"/>
<text x="449" y="141" text-anchor="middle" font-size="9.5" font-weight="600" fill="var(--fig-ink)">tile builder</text>
<text x="449" y="156" text-anchor="middle" font-size="9" fill="var(--fig-ink-soft)">2 attempts · 4 min</text>
<rect x="540" y="122" width="206" height="44" rx="6" fill="var(--fig-mint)" stroke="var(--fig-mint-edge)" stroke-width="1.4"/>
<text x="552" y="140" font-size="9" font-weight="600" fill="var(--fig-ink)">12 calls, worst case</text>
<text x="552" y="156" font-size="8.5" fill="var(--fig-ink-soft)">a 10× reduction in outage amplification</text>
<rect x="14" y="182" width="732" height="50" rx="6" fill="var(--fig-gold)" stroke="var(--fig-gold-edge)" stroke-width="1.3"/>
<text x="26" y="200" font-size="10" font-weight="600" fill="var(--fig-ink)">Each inner window must be strictly smaller than its caller's</text>
<text x="26" y="214" font-size="9" fill="var(--fig-ink-soft)">If an inner retry can still be in flight when the outer one gives up and retries, the outer attempt starts a second inner chain on top</text>
<text x="26" y="225" font-size="9" fill="var(--fig-ink-soft)">of the first — and the two ladders interleave into load neither service budgeted for.</text>
</svg>
<figcaption><b>Figure 3.</b> Budgets compose multiplicatively, so a per-hop limit that looks conservative in isolation is not. Size the end-to-end call count first, then divide it down the chain.</figcaption>
</figure>
