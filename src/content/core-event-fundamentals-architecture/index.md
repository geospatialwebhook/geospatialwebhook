...
<figure class="fig">
<svg viewBox="0 26 840 328" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Geospatial event pipeline overview: from spatial mutation through a normalise-and-validate layer into a partitioned broker and out to downstream consumers">
  ...
  <rect x="0" y="26" width="840" height="328" fill="var(--fig-bg)"/>
  <defs>
...
<figcaption><b>Figure 1.</b> Geospatial Event Pipeline Overview</figcaption>
</figure>
...
<figure class="fig">
<svg viewBox="0 0 760 286" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="The four layers of a spatial event envelope and which pipeline stage reads each one">
<title>The four layers of a spatial event envelope</title>
<desc>An annotated envelope split into four stacked bands. Event metadata carries event id, occurred at and schema version, and is read by the idempotency and replay stages. Spatial context carries the geometry in EPSG:4326 plus precomputed H3, S2 and quadkey index keys, and is read by validation. Payload data carries the attribute delta and is read only by the consumer. Routing instructions carry the topic and partition key and are read by the broker alone, which never parses the geometry.</desc>
<rect x="0" y="0" width="760" height="286" fill="var(--fig-bg)"/>
<defs>
<marker id="an-arr" markerWidth="8" markerHeight="6" refX="7" refY="3" orient="auto">
<path d="M0,0 L8,3 L0,6 Z" fill="var(--fig-line)"/>
</marker>
</defs>
<text x="14" y="20" font-size="11" font-weight="600" fill="var(--fig-ink)">One event envelope</text>
<text x="472" y="20" font-size="11" font-weight="600" fill="var(--fig-ink)">Read by</text>
<rect x="14" y="30" width="430" height="52" rx="7" fill="var(--fig-earth)" stroke="var(--fig-earth-edge)" stroke-width="1.4"/>
<text x="28" y="50" font-size="11" font-weight="600" fill="var(--fig-ink)">1 · Event metadata</text>
<text x="28" y="68" font-size="10" fill="var(--fig-ink-soft)">event_id · occurred_at · source · schema_version</text>
<rect x="14" y="90" width="430" height="66" rx="7" fill="var(--fig-mint)" stroke="var(--fig-mint-edge)" stroke-width="1.4"/>
<text x="28" y="110" font-size="11" font-weight="600" fill="var(--fig-ink)">2 · Spatial context</text>
<text x="28" y="128" font-size="10" fill="var(--fig-ink-soft)">geometry in EPSG:4326 (canonical, one CRS)</text>
<text x="28" y="145" font-size="10" fill="var(--fig-ink-soft)">h3_r7 · s2_cell · quadkey — precomputed at publish</text>
<rect x="14" y="164" width="430" height="52" rx="7" fill="var(--fig-peach)" stroke="var(--fig-peach-edge)" stroke-width="1.4"/>
<text x="28" y="184" font-size="11" font-weight="600" fill="var(--fig-ink)">3 · Payload data</text>
<text x="28" y="202" font-size="10" fill="var(--fig-ink-soft)">attribute delta · sensor reading · tile status</text>
<rect x="14" y="224" width="430" height="52" rx="7" fill="var(--fig-gold)" stroke="var(--fig-gold-edge)" stroke-width="1.4"/>
<text x="28" y="244" font-size="11" font-weight="600" fill="var(--fig-ink)">4 · Routing instructions</text>
<text x="28" y="262" font-size="10" fill="var(--fig-ink-soft)">topic · partition_key · consumer group · priority</text>
<line x1="448" y1="56" x2="516" y2="56" stroke="var(--fig-line)" stroke-width="1.3" marker-end="url(#an-arr)"/>
<text x="522" y="53" font-size="10" fill="var(--fig-ink)">Idempotency + replay</text>
<text x="522" y="67" font-size="9" fill="var(--fig-ink-soft)">dedup store, DLQ re-injection</text>
<line x1="448" y1="123" x2="516" y2="123" stroke="var(--fig-line)" stroke-width="1.3" marker-end="url(#an-arr)"/>
<text x="522" y="120" font-size="10" fill="var(--fig-ink)">Validation + routing</text>
<text x="522" y="134" font-size="9" fill="var(--fig-ink-soft)">topology check, shard selection</text>
<line x1="448" y1="190" x2="516" y2="190" stroke="var(--fig-line)" stroke-width="1.3" marker-end="url(#an-arr)"/>
<text x="522" y="187" font-size="10" fill="var(--fig-ink)">Consumer only</text>
<text x="522" y="201" font-size="9" fill="var(--fig-ink-soft)">the sole layer a broker ignores</text>
<line x1="448" y1="250" x2="516" y2="250" stroke="var(--fig-line)" stroke-width="1.3" marker-end="url(#an-arr)"/>
<text x="522" y="247" font-size="10" fill="var(--fig-ink)">Broker only</text>
<text x="522" y="261" font-size="9" fill="var(--fig-ink-soft)">never parses the geometry</text>
</svg>
<figcaption><b>Figure 2.</b> The four layers of a spatial event envelope. Layers 1, 2 and 4 exist so that the infrastructure — dedup store, validator, broker — can do its job without ever deserialising layer 3.</figcaption>
</figure>
...
<figure class="fig">
<svg viewBox="0 0 760 300" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Normalising coordinate reference systems at the producer boundary versus letting every consumer reproject">
<title>Where the reprojection cost lands: producer boundary versus consumer</title>
<desc>Two arrangements of the same pipeline. Above, three producers publish in EPSG:27700, EPSG:3857 and EPSG:4326, the stream carries mixed CRS, and each of the three consumers must reproject, so the transform runs nine times and any consumer that forgets misaligns silently. Below, each producer normalises to EPSG:4326 once at its own boundary, the stream is single-CRS, and the three consumers read directly, so the transform runs three times and a mislabelled CRS is rejected at the boundary rather than discovered downstream.</desc>
<rect x="0" y="0" width="760" height="300" fill="var(--fig-bg)"/>
<defs>
<marker id="crs-arr" markerWidth="8" markerHeight="6" refX="7" refY="3" orient="auto">
<path d="M0,0 L8,3 L0,6 Z" fill="var(--fig-line)"/>
</marker>
</defs>
<text x="14" y="18" font-size="11" font-weight="600" fill="var(--fig-rose-edge)">Reproject at the consumer — 9 transforms</text>
<rect x="14" y="28" width="96" height="26" rx="5" fill="var(--fig-rose)" stroke="var(--fig-rose-edge)" stroke-width="1.2"/>
<text x="62" y="45" text-anchor="middle" font-size="9.5" fill="var(--fig-ink)">EPSG:27700</text>
<rect x="14" y="60" width="96" height="26" rx="5" fill="var(--fig-rose)" stroke="var(--fig-rose-edge)" stroke-width="1.2"/>
<text x="62" y="77" text-anchor="middle" font-size="9.5" fill="var(--fig-ink)">EPSG:3857</text>
<rect x="14" y="92" width="96" height="26" rx="5" fill="var(--fig-rose)" stroke="var(--fig-rose-edge)" stroke-width="1.2"/>
<text x="62" y="109" text-anchor="middle" font-size="9.5" fill="var(--fig-ink)">EPSG:4326</text>
<line x1="112" y1="73" x2="176" y2="73" stroke="var(--fig-line)" stroke-width="1.3" marker-end="url(#crs-arr)"/>
<rect x="180" y="44" width="150" height="58" rx="7" fill="var(--fig-earth)" stroke="var(--fig-rose-edge)" stroke-width="1.6" stroke-dasharray="5,3"/>
<text x="255" y="66" text-anchor="middle" font-size="10.5" font-weight="600" fill="var(--fig-ink)">mixed-CRS stream</text>
<text x="255" y="83" text-anchor="middle" font-size="9" fill="var(--fig-ink-soft)">every event a different datum</text>
<line x1="332" y1="73" x2="392" y2="55" stroke="var(--fig-line)" stroke-width="1.2" marker-end="url(#crs-arr)"/>
<line x1="332" y1="73" x2="392" y2="73" stroke="var(--fig-line)" stroke-width="1.2" marker-end="url(#crs-arr)"/>
<line x1="332" y1="73" x2="392" y2="91" stroke="var(--fig-line)" stroke-width="1.2" marker-end="url(#crs-arr)"/>
<rect x="396" y="30" width="150" height="30" rx="5" fill="var(--fig-rose)" stroke="var(--fig-rose-edge)" stroke-width="1.2"/>
<text x="471" y="49" text-anchor="middle" font-size="9.5" fill="var(--fig-ink)">tile server + reproject</text>
<rect x="396" y="66" width="150" height="30" rx="5" fill="var(--fig-rose)" stroke="var(--fig-rose-edge)" stroke-width="1.2"/>
<text x="471" y="85" text-anchor="middle" font-size="9.5" fill="var(--fig-ink)">analytics + reproject</text>
<rect x="396" y="102" width="150" height="30" rx="5" fill="var(--fig-rose)" stroke="var(--fig-rose-edge)" stroke-width="1.2"/>
<text x="471" y="121" text-anchor="middle" font-size="9.5" fill="var(--fig-ink)">alerting + reproject</text>
<text x="558" y="66" font-size="9.5" fill="var(--fig-rose-edge)" font-weight="600">3 producers x 3 consumers</text>
<text x="558" y="81" font-size="9" fill="var(--fig-ink-soft)">one consumer that forgets</text>
<text x="558" y="94" font-size="9" fill="var(--fig-ink-soft)">misaligns without erroring</text>
<line x1="14" y1="152" x2="746" y2="152" stroke="var(--fig-line-soft)" stroke-width="1"/>
<text x="14" y="176" font-size="11" font-weight="600" fill="var(--fig-mint-edge)">Normalise at the producer — 3 transforms</text>
<rect x="14" y="186" width="96" height="26" rx="5" fill="var(--fig-peach)" stroke="var(--fig-peach-edge)" stroke-width="1.2"/>
<text x="62" y="203" text-anchor="middle" font-size="9.5" fill="var(--fig-ink)">EPSG:27700</text>
<rect x="14" y="218" width="96" height="26" rx="5" fill="var(--fig-peach)" stroke="var(--fig-peach-edge)" stroke-width="1.2"/>
<text x="62" y="235" text-anchor="middle" font-size="9.5" fill="var(--fig-ink)">EPSG:3857</text>
<rect x="14" y="250" width="96" height="26" rx="5" fill="var(--fig-peach)" stroke="var(--fig-peach-edge)" stroke-width="1.2"/>
<text x="62" y="267" text-anchor="middle" font-size="9.5" fill="var(--fig-ink)">EPSG:4326</text>
<line x1="112" y1="199" x2="174" y2="212" stroke="var(--fig-line)" stroke-width="1.2" marker-end="url(#crs-arr)"/>
<line x1="112" y1="231" x2="174" y2="231" stroke="var(--fig-line)" stroke-width="1.2" marker-end="url(#crs-arr)"/>
<line x1="112" y1="263" x2="174" y2="250" stroke="var(--fig-line)" stroke-width="1.2" marker-end="url(#crs-arr)"/>
<rect x="178" y="202" width="152" height="58" rx="7" fill="var(--fig-mint)" stroke="var(--fig-mint-edge)" stroke-width="1.6"/>
<text x="254" y="224" text-anchor="middle" font-size="10.5" font-weight="600" fill="var(--fig-ink)">normalise once</text>
<text x="254" y="241" text-anchor="middle" font-size="9" fill="var(--fig-ink-soft)">to EPSG:4326, then validate</text>
<text x="254" y="254" text-anchor="middle" font-size="9" fill="var(--fig-ink-soft)">a wrong CRS fails here</text>
<line x1="332" y1="231" x2="392" y2="213" stroke="var(--fig-line)" stroke-width="1.2" marker-end="url(#crs-arr)"/>
<line x1="332" y1="231" x2="392" y2="231" stroke="var(--fig-line)" stroke-width="1.2" marker-end="url(#crs-arr)"/>
<line x1="332" y1="231" x2="392" y2="249" stroke="var(--fig-line)" stroke-width="1.2" marker-end="url(#crs-arr)"/>
<rect x="396" y="188" width="150" height="30" rx="5" fill="var(--fig-mint)" stroke="var(--fig-mint-edge)" stroke-width="1.2"/>
<text x="471" y="207" text-anchor="middle" font-size="9.5" fill="var(--fig-ink)">tile server — reads</text>
<rect x="396" y="224" width="150" height="30" rx="5" fill="var(--fig-mint)" stroke="var(--fig-mint-edge)" stroke-width="1.2"/>
<text x="471" y="243" text-anchor="middle" font-size="9.5" fill="var(--fig-ink)">analytics — reads</text>
<rect x="396" y="260" width="150" height="30" rx="5" fill="var(--fig-mint)" stroke="var(--fig-mint-edge)" stroke-width="1.2"/>
<text x="471" y="279" text-anchor="middle" font-size="9.5" fill="var(--fig-ink)">alerting — reads</text>
<text x="558" y="224" font-size="9.5" fill="var(--fig-mint-edge)" font-weight="600">one transform per producer</text>
<text x="558" y="239" font-size="9" fill="var(--fig-ink-soft)">consumers cannot forget a</text>
<text x="558" y="252" font-size="9" fill="var(--fig-ink-soft)">step they never perform</text>
</svg>
<figcaption><b>Figure 3.</b> The reprojection cost is the same transform either way — what changes is how many times it runs and, more importantly, whether forgetting it is possible at all.</figcaption>
</figure>
