  ...
  "@graph": [
    {
      ...
      "url": "https://geospatialwebhook.com/core-event-fundamentals-architecture/webhook-security-boundaries/preventing-replay-attacks-on-spatial-webhooks/",
      "datePublished": "2025-05-01",
      "dateModified": "2026-07-13",
    ...
    },
    {
      ...
      "itemListElement": [
        ...
        { "@type": "ListItem", "position": 3, "name": "Preventing Replay Attacks on Spatial Webhooks", "item": "https://geospatialwebhook.com/core-event-fundamentals-architecture/webhook-security-boundaries/preventing-replay-attacks-on-spatial-webhooks/" }
      ]
    },
    {
      "@type": "HowTo",
...
This is *not* the right tool when the operation is naturally read-only or already commutative, or when your only concern is a provider's own well-behaved retries. Benign duplicate suppression is idempotency deduplication, a different problem with a different TTL — see [Idempotency & Spatial Deduplication](/idempotency-spatial-deduplication/). The rest of this page is deliberate about keeping the two apart.
...
They share Redis mechanics but differ in intent and TTL. Replay prevention is adversarial: a short window (minutes) and reject on a duplicate. Idempotency dedup is benign: it absorbs a provider's own retries over a long window (hours to days) by returning the original result. Do not collapse them into one key with one TTL — see [Idempotency & Spatial Deduplication](/idempotency-spatial-deduplication/) for the dedup side.
...
- [Webhook Security Boundaries](/core-event-fundamentals-architecture/webhook-security-boundaries/) — parent overview of the trust model for geospatial event ingress, of which replay prevention is one gate
- [Signing Spatial Webhook Payloads with HMAC-SHA256](/core-event-fundamentals-architecture/webhook-security-boundaries/signing-spatial-webhook-payloads-with-hmac-sha256/) — how to construct the signature whose bytes must include the timestamp and delivery id
- [Idempotency & Spatial Deduplication](/idempotency-spatial-deduplication/) — the benign, long-window counterpart to adversarial replay prevention, for absorbing a provider's own retries
