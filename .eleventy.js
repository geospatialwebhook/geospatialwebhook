...
const CANONICAL_ORIGIN = "https://www.geospatialwebhook.com";
...
module.exports = function (eleventyConfig) {
  ...
  // Content links are already absolute in source; this covers template-generated
  // links (header nav, footer, breadcrumbs, related lists, homepage cards).
  eleventyConfig.addTransform("absoluteInternalLinks", function (content, outputPath) {
    const op = outputPath || (this.page && this.page.outputPath);
    if (!op || !op.endsWith(".html")) return content;
    return content.replace(
      /(<a\b[^>]*?\shref=")\/(?!\/)([^"]*")/g,
      `$1${CANONICAL_ORIGIN}/$2`
    );
  });
